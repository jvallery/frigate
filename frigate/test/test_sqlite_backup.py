...
class SqliteBackupTest(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with sqlite3.connect(backup) as backup_connection:
                ...
                self.assertEqual(
                    ...
                    backup_connection.execute("SELECT * FROM marker ORDER BY id").fetchall(),
                )
            connection.close()
    ...
    def test_backup_is_consistent_while_writes_continue(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            seed.execute("CREATE TABLE writes (sequence INTEGER PRIMARY KEY, payload BLOB)")
            seed.executemany(
            ...
            self.assertTrue(
                any(backup_started <= stamp <= backup_finished for stamp in commit_times)
            )
            with sqlite3.connect(backup) as backup_connection:
