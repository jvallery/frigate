...
def persist_object_description(
    ...
    if model_class is None:
        ...
        model_class = Event
    table = model_class._meta.table_name
    cursor = model_class._meta.database.execute_sql(
        f'UPDATE "{table}" '
    ...
    fresh = model_class.get_by_id(event_id)
...
def persist_review_metadata(
    ...
    if cursor.rowcount == 1:
        return ConditionalWriteResult.written
    fresh = model_class.get_by_id(review_id)
    data = fresh.data if isinstance(fresh.data, Mapping) else {}
...
class DescriptionRecoveryCoordinator:
    ...
    def tick(self) -> bool:
        ...
        if admission != RecoveryAdmission.retry_later:
            self._logger.warning(
                "GenAI recovery processor returned an invalid admission outcome for %s",
                selected_surface,
            )
