...
- The upstream mirror branches `dev` and `0.19`, advanced only by `.github/workflows/vallery-upstream-sync.yml`.
...
- Authoritative docs: `deploy/kubernetes/local-dev/README.md` governs the development runtime contract (parked state, what the tree renders, network, route, and metrics labels); `deploy/kubernetes/local-dev/DELIVERY.md` governs the release factory, promotion, rollback, and the Sentinel selection boundary; `.vallery/upstream-policy.json` and `.vallery/downstream-patches.json` govern mirrors, patch-sensitive paths, and carried patches.
...
- `dev` and `0.19` are automation-only fast-forward mirrors; never push to them. Upstream intake arrives as `automation/intake-*` PRs to `vallery/prod`.
...
- This `AGENTS.md` is a downstream rewrite of upstream's file (`CLAUDE.md` links to it; `.github/copilot-instructions.md` links to it as `../AGENTS.md`). An upstream change to it makes the intake merge conflict, and the sync fails closed; resolve by keeping this version.
- `deploy/kubernetes/ore-maintenance/` is a dormant, attended-operator state-restore protocol that CI still tests; do not activate the overlay.
